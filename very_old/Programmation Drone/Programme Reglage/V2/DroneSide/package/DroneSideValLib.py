...
class ValuesObject (object):
    def __init__ (self):
        ...
        self.vref_volt = 3.3
        self.vref = self.vref_volt * 1000 # V-Ref in mV (Vref = VDD for the MCP3002)
        self.resolution = 2**10 # for 10 bits of resolution
        self.calibration = 38 # in mV, to make up for the precision of the components

        self.mpu9250 = MPU9250()

    def setRegulation(self, *arg):#Kp, Ki, Kd):
        """
    ...
    def convertMag(self, magX, magY, magZ):
        """
        Convert the given values of the magnetometer and convert into angle
        """
        return (90 - atan(magY/magX) * 180/pi)
    ...
    def readMPU(self):
        ...
        """
        self.accX, self.accY, self.accZ = self.mpu9250.readAccel()
        self.gyroX, self.gyroY, gyroZ = self.mpu9250.readGyro()
        self.magX, self.magY, self.magZ = self.mpu9250.readMagnet()
        self.temperature = self.mpu9250.readTemperature()

        self.magAngle = convertMag(self.magX, self.magY, self.magZ)


    def readMCP3002(self, channel):
        """
        Read the analog value from the MCP3002
        usage : channeldata_1 = read_mcp3002(0)
        """
        ...
        #voltage = int(round(((adc_data * vref * 2) / self.resolution),0)) + self.calibration
        # Return custom formula to convert the value of the MCP3002 in Volt
        return adc_data*1000/1023 * self.vref_volt
